...
public sealed class CredentialTypesQueryHandler(IServiceProvider provider) : IQueryHandler<CredentialTypesQuery, Result<IEnumerable<CredentialTypeDto>>>
{
  private readonly IReadOnlyDbContext _context = provider.GetRequiredKeyedService<IReadOnlyDbContext>(ContextTypes.Identity);

  public Task<Result<IEnumerable<CredentialTypeDto>>> Handle(CredentialTypesQuery request, CancellationToken cancellationToken)
#pragma warning disable CS8620 // El argumento no se puede usar para el parámetro debido a las diferencias en la nulabilidad de los tipos de referencia.
    => Result.From(
        _context.Repository<CredentialType>().Select(row => new CredentialTypeDto(row.Id, row.Name, row.Description)).ToListAsync(cancellationToken),
        ResultTypes.NotFound,
        [new Error("credential-types not found", "no se encontraron tipos de credenciales")]
      )
