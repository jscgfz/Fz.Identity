...
public sealed class PtCredentialValidatorService(IServiceProvider provider) : ICredentialValidatorService
{
  ...
  public async Task<Result> CreateCredential(Guid userId, string username)
  {
    if (_context.Repository<Credential>()
      .AsEnumerable()
      .Where(row => row.UserId == userId && row.CredentialTypeId == (int)CredentialTypes.PtDomain && row.CredentialValue.TryGetProperty("Username", out JsonElement userName) && userName.GetString() == username)
      .Any())
  ...
  {
    int credentialType = ((int)CredentialTypes.PtDomain);
  ...
  private async Task<Result<User>> ValidateCredentials(CredentialDto credential)
  {
    ...
    HttpRequestMessage request = new(HttpMethod.Post, new Uri($"{_settings.Value.BaseUrl}RespuestaLgPt"))
    {
