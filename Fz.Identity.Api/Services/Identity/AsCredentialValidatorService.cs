...
public sealed class AsCredentialValidatorService(IServiceProvider provider) : ICredentialValidatorService
{
  ...
  private readonly IOptions<AsCredentialSettings> _settings = provider.GetRequiredService<IOptions<AsCredentialSettings>>();
  ...
  {
    int credentialType = ((int)CredentialTypes.AsDomain);
  ...
  private async Task<Result<User>> ValidateCredentials(CredentialDto credential)
  {
    ...
    HttpRequestMessage request = new(HttpMethod.Post, new Uri($"{_settings.Value.BaseUrl}ValidaUsuarioAsisYa"))
    {
